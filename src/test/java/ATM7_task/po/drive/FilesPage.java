...
public class FilesPage extends AbstractPage {
    ...
    private By files = By.cssSelector("div.js-prevent-deselect.js-prevent-mouse-selection");
    ...
    public FilesPage dragAndDropSquare() {
        ...
        new Actions(driver).clickAndHold(driver.findElement(file)).
                moveToElement(driver.findElement(trashIcon)).click().build().perform();
        return this;
    }

//    public FilesPage dragAndDropFile() {
//        waitForElementVisibility(nameOfFolder);
...
//    }
    ...
    public UploadPopup uploadFile() {
        waitForElementVisibility(trashIcon);
        driver.findElements(By.cssSelector("input.upload-button__attach")).get(0).sendKeys(new File("src\\test\\resources\\test.json").getAbsolutePath());
        return new UploadPopup();
    }

    public ActionBarPopup selectTwoFileFromList(){
        List<WebElement> listFiles = driver.findElements(files);
        waitForElementVisibility(files);
        Actions act=new Actions(driver);
        act.keyDown(Keys.CONTROL)
                .click(listFiles.get(0))
                .click(listFiles.get(1))
                .keyUp(Keys.CONTROL)
                .build()
                .perform();
    ...
    public ActionBarPopup clickToFirstFileFromList(){
        waitForElementVisibility(files);
        driver.findElements(files).get(0).click();
        return new ActionBarPopup();
    }
